...
// Selecione todas as imagens do site
var images = document.querySelectorAll('img');

// Defina a imagem de fallback que será carregada em caso de erro
var fallbackImage = '../public/assets/img/website/no-image.png';

// Adicione o evento onerror às imagens
images.forEach(function(image) {
  image.onerror = function() {
    // Carregue a imagem de fallback
...
// Selecione todos os botões que possuem o atributo delete-alert
const deleteButtons = document.querySelectorAll('[delete-alert]');

// Itere sobre cada botão de delete
deleteButtons.forEach(deleteButton => {
    const form = deleteButton.closest('form'); // Selecione o formulário mais próximo

    // Adicione um evento de clique ao botão de delete
    deleteButton.addEventListener('click', (e) => {
        // Previna o comportamento padrão do botão de delete
        e.preventDefault();


        // Verifique se o botão já foi clicado
        if (deleteButton.hasAttribute('open-alert-button')) {
            // Simule o envio do formulário
            form.dispatchEvent(new Event('submit', { bubbles: true }));
        } else {
            // Adicione o parâmetro open-alert-button ao botão

            // Obtenha o valor do atributo delete-alert
            const deleteAlertValue = deleteButton.getAttribute('delete-alert');
            const messageWithAlertCode = deleteButton.getAttribute('messageWithAlertCode') === "true";
            const redirect = deleteButton.getAttribute('redirect');
            const AlertMessage = deleteButton.getAttribute('AlertMessage');

            let messageSwal;

            if(AlertMessage) { 
                messageSwal = AlertMessage;
                
            } else if (messageWithAlertCode === true) {
                messageSwal = `Se clicar em <b>Apagar</b> a transação ${deleteAlertValue}, será automaticamente eliminada!`;
            } else {
                messageSwal = 'Ao clicar em <b>Apagar</b>, o artigo, será automáticamente apagado.'
            }

            // Crie uma instância do SweetAlert2
            Swal.fire({
                ...
                confirmButtonColor: '#dc3545', // Botão de confirmação em vermelho


            }).then((result) => {
                // Se o usuário confirmar, envie o formulário
                if (result.isConfirmed) {
                    // Remova o parâmetro open-alert-button do botão
                    deleteButton.removeAttribute('open-alert-button');

                    // Simule o envio do formulário
                   window.location = redirect;
                }
            }).finally(() => {
                // Adiciono para se clicar novamente no botão o swal for aberto
                deleteButton.removeAttribute('open-alert-button');
            });
        }
    });
